...
def main():
    ...
    temperature = 0.9      # 1.0~1.3 추천
    rest_logit_penalty = 2.0   # 1.0~4.0 추천 (클수록 rest 덜 나옴)
    ...
    PITCH_MIN, PITCH_MAX = 40, 76
    long_tokens = long_tokens.copy()
    mask = long_tokens != REST_TOKEN
    long_tokens[mask] = np.clip(long_tokens[mask], PITCH_MIN, PITCH_MAX)
    
    drop_prob = 0.25
    rng = np.random.default_rng(int(time.time()) % (2**32 - 1))
    mask_note = (long_tokens != REST_TOKEN)
    drop = rng.random(len(long_tokens)) < drop_prob
    long_tokens[mask_note & drop] = REST_TOKEN
