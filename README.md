...
4.토큰 정의:
 ...
 Verse 구간의 기타 연주는  
 모든 step마다 음이 존재하기보다는 음이 울리며 지속되는 구간과  
 ...
 추가적인 인코딩 없이 음높이 정보가 직접적으로 보존되며 
...
- 목적: rest 토큰이 “너무 쉽게” 선택되는 것을 막기위해
- 효과: 음이 실제로 생성될 확률 증가
...
결과 MIDI 파일에서도 노트가 생성되어 실제로 연주라고 볼 수 있는 기타 리프를 얻을 수 있었다.

또한 이 과정은 단순히 오류수정이 아니라
소규모 데이터 환경에서 생성 모델이 쉽게 붕괴할 수 있다는 점을 확인할 수 있었고
이를 디코딩 단계에서 안정화하는 방법또한 알게 되었습니다.

---
#### 결과2
하지만 그래도 기타연주와는 다른 느낌이들어서 다음 과정을 추가하였습니다.

###  마디 단위 화성 결정 (Bar-Level Harmony)
본 모델은 beat 단위가 아닌 **마디 단위로 코드 중심음을 결정**한다.

- 한 마디 동안 같은 코드 루트를 유지
- 여러 번 반복해서 연주하되, 연주 방식만 변화

이는 기타 연주에서 흔한
> “코드 하나를 잡고 여러 번 치는 방식”
을 그대로 반영한 설계이다.

---

###  고정 리듬 + 보이싱 변화
- 곡 전체에서 **하나의 리듬 패턴을 랜덤으로 선택**
- 해당 리듬은 곡이 끝날 때까지 고정
- 대신, 각 리듬 히트마다 **보이싱 순서만 변화**
...
###  Voicing Cycle (기타 특화 보이싱)
한 마디 안에서 음을 고르는 순서를 문자열 패턴으로 정의:
...
의미:
- `1,2,3` → 코드 톤 중 특정 음
- `4` → 상단 2음을 동시에 치는 **더블 스탑**

이는 기타 핑거피킹/스트럼에서 자주 쓰이는
연주 습관을 추상화한 것이다.

---

###  코드 톤 변형 혼합 (단조로움 방지)
같은 코드라도 매번 같은 음만 치지 않도록,
각 리듬 히트마다 코드 톤 구성을 조금씩 바꾼다.
...
이를 통해 여러 번 생성해도
항상 같은 느낌의 시작을 피할 수 있다.

---

###  기타 전용 MIDI 렌더링
MIDI 출력 또한 기타 연주에 맞게 설계하였다.

- 인위적인 악센트 제거
- 미세한 타이밍 흔들림으로 사람 손맛 구현
- 노트 오버랩으로 현 울림 표현
- 저역/옥타브 보강 제거 (피아노화 방지)
